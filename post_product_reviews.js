...
- [가격 비싸다 리뷰] → 가격 방어:
  → "안미끌리고 디자인 예쁘다"는 만족 포인트 인정 + 워셔블 레더의 차별성·세탁 편의성으로 자연스럽게 가치 어필
  → 핵심 디펜스 맥락: "**수많은 테스트를 거쳐 직접 개발한 소재**"이며, "**오염에 강하면서도 물기를 흡수하는 발매트 소재는 시중에서 찾기 힘들다**"는 점 강조. "오래 쓸수록 가치를 느끼실, **후회 없는 선택**"이라는 톤으로 마무리
...
- 주의: 'KC인증' 표현보다 'KC 마크를 획득한 제품' 이 더 정확`);
  ...
  // ── 샤워기·필터 라인 ────────────────────────────
  // '샤워 세트/스타터/올인원' 등 세트 제품명도 PLA 샤워기 지식을 받도록 포함 (예: '올인원 샤워 세트')
  if (name.includes('샤워기') || (name.includes('샤워') && (name.includes('세트') || name.includes('스타터') || name.includes('올인원')))) {
...
- 🚨 ["무겁다 / 무게가 있다 / 손목 아프다" 류 무게 불만 — 방어 필수, 그냥 넘기지 말 것]
  → 방어 논리: 바디 필터·헤드 필터 등 필터가 2개(듀얼) 내장되고 원터치 온오프 기능까지 더해지다 보니 조금 묵직하게 느껴질 수 있으나, **일반 샤워기와 실제 무게 차이는 크지 않음**
  → 특정 상황(예: 출산 직후·손목이 약하실 때)에 더 민감하게 느끼실 수 있음을 부드럽게 공감
  → 🚨 절대 "제품팀에 전달하겠다"로만 넘기지 말 것. 반드시 위 논리로 방어한 뒤 건강·안심 마무리로 공감`);
  }
...
- 샤워기/필터 라인: **100% 식물 유래 PLA 소재 → 미세플라스틱이 검출되지 않음** (일반 플라스틱 샤워기·필터는 미세플라스틱이 배출됨). 잔류 염소·녹물·세균까지 듀얼 필터로 차단
  - 🚨 "무겁다/손목 아프다" 류 무게 불만은 방어 필수: 필터 2개(듀얼)+온오프 기능으로 조금 묵직할 수 있으나 일반 샤워기와 무게 차이 크지 않음 + 출산 직후 등 상황상 더 민감할 수 있음 공감. "제품팀 전달"로만 넘기지 말 것
...
async function generateWordDoc(summary) {
  ...
  const children = [
    ...
    ...(summary.verificationSummary ? [
      ...
      ...summary.verificationSummary.split('\n').map(line => {
        const urgent = line.includes('🚨');  // 2일+ 미처리 경고 라인은 빨강 bold
