...
1. **환불검토 빗나감** (verdict='빗나감', 즉 블라인드 안 되고 정상+답글 Y 로 답변처리됨)
   → 내가 환불(블라인드)로 제안했으나 담당자가 **답변(교환·안내)으로 처리**한 것 = 너무 보수적.
   → **반드시** 그 건(reviewNo·제품·내 판단근거·실제 처리)을 `memory/feedback_reply_rules.md` 의
     **"## 🧠 환불검토 사후검증 학습 노트"** 섹션에 한 항목으로 누적 기록한다. (빠뜨리지 말 것)
2. **답변 빗나감** (verdict='빗나감(환불됨)', 즉 답변으로 봤는데 실제 환불됨)
   → 내가 환불검토로 올렸어야 할 건을 답변으로 본 것. 더 적극적으로 봤어야. 동일 섹션에 누적.
3. 위 학습 노트는 Step 0 풀 지식 덤프 시 함께 로드되어 다음 판단에 반영됨.
   generate_report 도 verification_result.json 에서 빗나감 건을 자동으로 "📝 학습 노트" 라인으로
   검증 요약에 포함하므로, verification_summary.txt 를 직접 쓸 때도 빗나감 건은 학습 노트 라인을 반드시 포함.
